...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         ...
         isRecord, ProcTExp, makeUserDefinedNameTExp, Field, makeAnyTExp, isAnyTExp, isUserDefinedNameTExp, UserDefinedNameTExp } from "./TExp";
...
export const typeofLit = (exp: LitExp, _tenv: TEnv, _p: Program): Result<TExp> => 
    typeof (exp.val) === 'string' ? makeOk(makeStrTExp()) :
        typeof (exp.val) === 'number' ? makeOk(makeNumTExp()) :
            typeof (exp.val) === 'boolean' ? makeOk(makeStrTExp()) :
                isPrimOp(exp.val) ? typeofPrim(exp.val) :
                    isClosure(exp.val) ? typeofExp(exp.val.body[exp.val.body.length - 1], _tenv, _p) :
                        isSymbolSExp(exp.val) ? applyTEnv(_tenv, exp.val.val) : makeFailure(`Todo ${JSON.stringify(exp, null, 2)}`);
