...
export const initTEnv = (p: Program): TEnv => {
    ...
    const procs3 = typeDefinitions.map( x => makeProcTExp([makeAnyTExp()], makeBoolTExp()));
    const nameprocs3 = typeDefinitions.map(rec => rec.typeName + '?');
...
const checkUserDefinedTypes = (p: Program): Result<true> =>{
    ...
    const UserDefined = getTypeDefinitions(p);
    const func2 = (usdef: UserDefinedTExp) => {
        const BaseExists = usdef.records.some((record) => record.fields.every((y) => y.te.tag !== usdef.tag));
        return BaseExists;
    }
    const records = getRecords(p);
    const func = (record: Record) => {
        const parents = getRecordParents(record.typeName, p).map(x => x.records.filter(y => y.typeName === record.typeName)[0].fields);
        const allEqual = parents.every(x => equals(x, record.fields));
        if (allEqual) {
            return true;
        }
        return false;
    }
    const finalAnswer = records.every(func) && UserDefined.every(func2);
    if (finalAnswer) {
        return makeOk(true);
    }
    return makeFailure("3.2,4.2");
}
...
export const checkTypeCase = (tc: TypeCaseExp, p: Program): Result<true> => {
    ...
    // (in any order)
    const a = getUserDefinedTypeByName(tc.typeName, p);
    if(isOk(a)){
...
// TODO L51
export const typeofLit = (exp: LitExp, _tenv: TEnv, _p: Program): Result<TExp> =>makeOk(makeLitTExp());
